...
# Adam(net.parameters(), betas=(0.9, 0.98), eps=1e-9)
...
for epoch in tqdm(range(args.epoch_num)):
    ...
    if args.use_weightwatcher:
        ...
        if args.baseline:
            optimizer = Adam(model.parameters(),lr = get_lr(epoch, args.lr, args.epoch_num),betas=(0.9, 0.98), eps=1e-9)
            if args.record_lr:
            ...
            if args.average:
                ...
                #     para_copy.append({
                #         'params':net_para1[sorted_index[i]],
                #         'lr': (1 - args.percentage) * get_lr(epoch, args.lr, args.epoch_num),
                #     })
            ...
            optimizer = Adam(para_save, 
                                lr = get_lr(epoch, args.lr, args.epoch_num),betas=(0.9, 0.98), eps=1e-9)
