...
class Transformer(nn.Module):
    ...
    def __init__(self, input_dim, embedding_dimension, model_dimension, number_of_heads, number_of_layers, dropout_probability, log_attention_weights=False):
        super().__init__()

        # Embeds source/target token ids into embedding vectors
        self.embedding = nn.Embedding(input_dim, embedding_dimension, padding_idx = 1)
        ...
        self.src_pos_embedding = PositionalEncoding(embedding_dimension, dropout_probability)
        ...
        # self.decoder = nn.Linear(model_dimension, src_vocab_size)
        # self.decoder = Decoder(decoder_layer, number_of_layers)
        self.max_pool = nn.MaxPool1d(model_dimension, stride=1)
        ...
        self.init_params()
        # initrange = 0.1
        # nn.init.uniform_(self.src_embedding.embeddings_table.weight, -initrange, initrange)
    ...
    def forward(self, src_token_ids_batch, src_mask = None, trg_mask = None):
        # ipdb.set_trace()
        # device = src_token_ids_batch.device
        # ipdb.set_trace()
        
        src_mask, num_src_tokens = get_masks_and_count_tokens_src(src_token_ids_batch, 1)
        # print(src_mask[:, :, :, -1])
        # bs, seq_len = src_token_ids_batch.shape[0], src_token_ids_batch.shape[1]
        # src_mask = (torch.triu(torch.ones(seq_len, seq_len)) == 1).transpose(0, 1).float()
        # src_mask = src_mask.repeat((bs, 1, 1, 1)).cuda()
        output = self.encode(src_token_ids_batch, src_mask)
        # [batch size, seq, dim]
        # print('encoder', output.shape)
        # decoded = self.decoder(output)
        # ipdb.set_trace()
        bs = output.shape[0]                    
        output = output.permute(0, 2, 1).contiguous()       #[64, 100, 500]
        decoded = F.avg_pool1d(output, kernel_size = output.shape[-1]).view(bs, -1)     #[64, 100]
        decoded = self.decoder(decoded)
        
        # trg_log_probs = self.decode(trg_token_ids_batch, src_representations_batch, trg_mask, src_mask)
        return decoded#.permute(1, 0, 2).contiguous()
    ...
    def encode(self, src_token_ids_batch, src_mask):
        src_embeddings_batch = self.embedding(src_token_ids_batch)  # get embedding vectors for src token ids
...
class PositionwiseFeedForwardNet(nn.Module):
    ...
    def __init__(self, model_dimension, dropout_probability, width_mult=4):
        ...
        self.linear1 = nn.Linear(model_dimension, model_dimension)
        self.linear2 = nn.Linear(model_dimension, model_dimension)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, embeddings_batch):
        ...
        return embeddings_batch + positional_encodings
...
if __name__ == "__main__":
    ...
    src_vocab_size = 11
    trg_vocab_size = 11
    src_token_ids_batch = torch.randint(1, 10, size=(20, 64))
    ...
    out = transformer(src_token_ids_batch, src_mask=None, trg_mask=None)
